...
public final class LcdController implements Component, Clocked {
    ...
    public int read(int address) {
        ...
        if (address >= REGS_LCDC_START && address < REGS_LCDC_END) {
            return registerFile.get(Reg.values()[address - REGS_LCDC_START]);
        }
    ...
    private int lsbAddress(int row, int col, boolean background) {
        ...
        int indexAddress = (r / TILE_HEIGHT) * (IMAGE_HEIGHT / TILE_HEIGHT)
                + col / TILE_WIDTH
                + BG_DISPLAY_DATA[registerFile.testBit(Reg.LCDC,
        ...
        int tileAddress = TILE_SOURCE[source ? 1 : 0] + tileIndex * 0x10
                + (source ? 0
                        : tileIndex >= TILE_SOURCE[1] / 0x100
                                ? -TILE_SOURCE[1] / 0x10
                                : TILE_SOURCE[1] / 0x10);
