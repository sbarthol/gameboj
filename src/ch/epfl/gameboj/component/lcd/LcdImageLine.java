...
public final class LcdImageLine {
    ...
    public LcdImageLine(BitVector lsb, BitVector msb, BitVector opacity) {
        ...
        checkArgument(lsb.size() == msb.size() && lsb.size() == opacity.size());
        this.msb = msb;
        this.lsb = lsb;
    ...
    public LcdImageLine mapColors(int color) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            new_lsb = new_lsb.or(new BitVector(size(), test(color, 2 * i))
                    .and(lsb.xor(new BitVector(size(), !test(i, 0))))
                    .and(msb.xor(new BitVector(size(), !test(i, 1)))));

            new_msb = new_msb.or(new BitVector(size(), test(color, 2 * i + 1))
                    .and(lsb.xor(new BitVector(size(), !test(i, 0))))
                    .and(msb.xor(new BitVector(size(), !test(i, 1)))));
        }
    ...
    public LcdImageLine below(LcdImageLine upper, BitVector opacity) {
        ...
        checkArgument(upper.size() == size() && opacity.size() == size());
    ...
    public LcdImageLine join(LcdImageLine other, int index) {
        ...
        checkArgument(other.size() == size() && index >= 0 && index <= size());
