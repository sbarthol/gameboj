...
public final class Bits {
    ...
    private static final int[] table = new int[] { 0x00, 0x80, 0x40, 0xC0, 0x20,
    ...
    public static boolean test(int bits, int index) {
        ...
        checkIndex(index, Integer.SIZE);
        return (bits & Bits.mask(index)) != 0;
    }

    /**
     * Se comporte comme la méthode précédente mais obtient l'index à tester du
     * bit donné
     */
     ...
     * true à vrai) ; lève IndexOutOfBoundsException si l'index est invalide
     */
    ...
    public static int clip(int size, int bits) {
        ...
        return bits & (size == 32 ? -1 : mask(size) - 1);
    }
    ...
    public static int extract(int bits, int start, int size) {
        ...
        if (size >= 0 && start >= 0 && size + start <= Integer.SIZE) {
            return (bits >> start) & (size == 32 ? -1 : mask(size) - 1);
        } else {
    ...
    public static int rotate(int size, int bits, int distance) {
        ...
        distance = ((distance % size) + size) % size;
        return extract(bits, size - distance, distance)
                | (clip(size - distance, bits) << distance);
    }

    /**
     * « Etend le signe » de la valeur 8 bits donnée, c-à-d copie le bit d'index
     ...
     * deux de lowB ; lève IllegalArgumentException si l'une des deux valeurs
